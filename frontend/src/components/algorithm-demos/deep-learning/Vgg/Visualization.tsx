...
interface TrainingResult {
  ...
  conv_blocks?: Record<string, unknown>[];
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          ...
          <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
            <p>Model: {(model_info?.model_variant as string) || 'vgg16'}</p>
            <p>
              Parameters: {new Intl.NumberFormat().format(
                (model_info?.total_parameters as number) || 0
              )}
            </p>
            <p>Depth: {(model_info?.depth as number) || 16}</p>
            <p>
              Conv Layers: {(model_info?.num_conv_layers as number) || 13}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <p className="text-sm text-blue-900 dark:text-blue-100">
          <strong>Interpretation:</strong> VGG networks use uniform 3x3 convolutions stacked
          in sequence within convolutional blocks. This systematic approach is easy to understand
          and provides excellent feature learning for image classification tasks, though they
          are more computationally expensive than ResNet due to their larger parameter count.
        </p>
      </div>
    </div>
  );
