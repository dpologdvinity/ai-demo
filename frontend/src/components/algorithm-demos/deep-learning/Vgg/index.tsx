...
export function VggDemo() {
  const [parameters, setParameters] = useState<VGGParams>({
    model_variant: 'vgg16',
    top_k: 5,
    use_pretrained: true,
    batch_norm: true,
  ...
  const { data: algorithmInfo, isLoading: isLoadingInfo } = useQuery({
    queryKey: ['vgg-info'],
    queryFn: () => apiService.getAlgorithmInfo('deep-learning', 'vgg'),
  });

  const trainMutation = useMutation({
    mutationFn: (params: VGGParams) =>
      apiService.trainAlgorithm('deep-learning', 'vgg', params),
    onSuccess: (data) => {
      setResult(data);
    },
  });
  ...
  const handleParameterChange = (name: keyof VGGParams, value: unknown) => {
    setParameters((prev) => ({
      ...prev,
      [name]: value,
    }));
  ...
  return (
    <AlgorithmLayout
      title="VGG (Visual Geometry Group Networks)"
      description="Deep convolutional networks with uniform 3x3 convolutions"
                ...
                <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  ...
                  <div className="space-y-1 text-sm text-green-800 dark:text-green-200">
                    ...
                    <p>
                      ...
                      {(result.model_info?.model_variant as string) || 'vgg16'}
                    </p>
                  </div>
                </div>
              )}
