...
export function GruDemo() {
  const [parameters, setParameters] = useState<GRUParams>({
    hidden_size: 64,
    num_layers: 1,
    learning_rate: 0.001,
    epochs: 50,
  ...
  const { data: algorithmInfo, isLoading: isLoadingInfo } = useQuery({
    queryKey: ['gru-info'],
    queryFn: () => apiService.getAlgorithmInfo('deep-learning', 'gru'),
  });

  const trainMutation = useMutation({
    mutationFn: (params: GRUParams) =>
      apiService.trainAlgorithm('deep-learning', 'gru', params),
    onSuccess: (data) => {
      setResult(data);
    },
  });
  ...
  const handleParameterChange = (name: keyof GRUParams, value: any) => {
    setParameters((prev) => ({
      ...prev,
      [name]: value,
    }));
  ...
  return (
    <AlgorithmLayout
      ...
      description="Simplified sequential model with fewer parameters than LSTM"
              ...
              {trainMutation.isPending ? (
                ...
                <div className="flex items-center justify-center h-96 text-gray-500 dark:text-gray-400">
                  <div className="text-center">
                    ...
                    <p className="text-sm">
                      Configure parameters and click "Train GRU" to see training curves and predictions
                    </p>
                  </div>
                </div>
              )}
