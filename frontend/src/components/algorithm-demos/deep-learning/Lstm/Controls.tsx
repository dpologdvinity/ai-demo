...
interface ControlsProps {
  ...
  onChange: (name: keyof LSTMParams, value: any) => void;
  algorithmInfo?: any;
}
...
export function Controls({ parameters, onChange }: ControlsProps) {
  return (
    <div className="space-y-4">
      ...
      <ParameterControl
        label="Hidden Size"
        description="Memory cell dimension (64-512)"
        type="range"
      ...
      <ParameterControl
        ...
        description="Stacked LSTM layers (1-4)"
        type="range"
      ...
      <ParameterControl
        ...
        description="Input sequence length (10-100)"
        type="range"
      ...
      <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <p className="text-sm text-blue-900 dark:text-blue-100">
          <strong>Task:</strong> Time series forecasting with LSTM memory cells
        </p>
        <p className="text-xs text-blue-800 dark:text-blue-200 mt-1">
          LSTM learns long-term dependencies in sequential patterns
        </p>
      </div>
    </div>
  );
