...
export function LinearRegressionDemo() {
  const [parameters, setParameters] = useState<LinearRegressionParams>({
    fit_intercept: true,
    normalize: true,
    dataset_name: 'boston',
    test_size: 0.3,
  ...
  const { data: algorithmInfo, isLoading: isLoadingInfo } = useQuery({
    queryKey: ['linear-regression-info'],
    queryFn: () => apiService.getAlgorithmInfo('ml', 'linear-regression'),
  });

  // Training mutation
  const trainMutation = useMutation({
    mutationFn: (params: LinearRegressionParams) =>
      apiService.trainAlgorithm('ml', 'linear-regression', params),
    onSuccess: (data) => {
      setResult(data);
    },
  });
  ...
  const handleParameterChange = (name: keyof LinearRegressionParams, value: any) => {
    setParameters((prev) => ({
      ...prev,
      [name]: value,
    }));
